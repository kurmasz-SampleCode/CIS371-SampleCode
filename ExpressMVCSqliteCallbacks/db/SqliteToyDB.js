...
class SqliteToyDB {
    ...
    static create(description, callback) {
        ...
        if (newToy.isValid()) {
            ...
            this.sqliteDB.run(`INSERT INTO Toys (name, description, manufacturer, price) VALUES ("${newToy.name}", "${newToy.description}", "${newToy.manufacturer}", "${newToy.price}")`,
                function(_err, _data) {
    ...
    static update(toy) {
        this.sqliteDB.run(`UPDATE Toys SET name="${toy.name}", price="${toy.price}", description="${toy.description}", manufacturer="${toy.manufacturer}" where id="${toy.id}"`)
    }

    //////////////////////////////////////////////////////////
    // 
    // Demonstration of potential "Callback Hell"
    // 
    //////////////////////////////////////////////////////////
...
SqliteToyDB.sqliteDB = new sqlite3.Database(__dirname + '/toys.sqlite')
