...
function AuthorForm({ author, updateAuthor, formMode, submitCallback }) {
    ...
    if (formMode === "new") {
      ...
      return (
        <div className="form-group">
          ...
          <button type="submit" className="btn btn-danger" onClick={(event) => event.preventDefault()} >Cancel</button>
        </div>
      );
    }
  } // end renderButtons
...
function AuthorListItem({ author, onEditClicked }) {
...
function AuthorList({ authors, onEditClicked }) {
  const authorItems = authors.map((author) => (
    <AuthorListItem key={author.id} author={author} onEditClicked={onEditClicked} />
  ));
...
function Authors() {
  ...
  let [currentAuthor, setCurrentAuthor] = React.useState({ fname: '', lname: '', email: '' });
  ...
  let formSubmitted = () => {
    if (formMode === "new") {
      currentAuthor.id = authorList.length + 1;
      ...
      if (false) {
        console.log("Here!");
        authorList[currentAuthor.id - 1] = currentAuthor;
  ...
  return (
    <div className="authors">
      <AuthorForm formMode={formMode} author={currentAuthor} updateAuthor={updateAuthor} submitCallback={formSubmitted} />
      <div />
      <AuthorList authors={authorList} onEditClicked={editClicked} />
    </div>
  );
}
